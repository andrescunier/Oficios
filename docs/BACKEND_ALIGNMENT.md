...
| Chat mediación | `POST/GET /api/accounts/{account_id}/conversations...` (APIs nuevas) | Bearer customer/supplier | hilo por orden aceptada; plataforma modera | `conversationService` | backend contract | `MediationChat` en pedidos/reservas |
| Capacitaciones | `GET/PATCH /api/accounts/{account_id}/tasks/mine` | Bearer supplier | tareas `metadata.project=capacitacion`; embeds en `metadata.content_blocks` | `taskService` | backend contract | `ProviderDashboard` capacitaciones |
| Capacitaciones (gestión) | `GET/POST/PATCH /api/accounts/{account_id}/capacitaciones` (+ `/providers`) | Bearer platform (admin/operator/owner) / supplier lectura | API nueva; crea UserTask por proveedor con `content_blocks` | `capacitacionService` + `CapacitacionManager` | backend contract | `ProviderDashboard` tab Capacitaciones |
